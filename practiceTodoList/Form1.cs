namespace practiceTodoList
{
    public partial class Form1 : Form
    {
        public Form1()
        {
            ...
            //---Dirty=編集中で何か書いているが、未確定という意味
            todoGridView.CurrentCellDirtyStateChanged += (s, e) =>  //CurrentCellDirtyStateChangedは、現在のセルがDirtyになったら発火
            {
                if (todoGridView.IsCurrentCellDirty)    //現在のセルがDirtyなら
                {
                    todoGridView.CommitEdit(DataGridViewDataErrorContexts.Commit);  //その変更を確定させた時に、CellValueChangedイベントを発火
                }
            };
        ...
        {
            todoGridView.Columns.Clear();   //初期化

            todoGridView.AutoGenerateColumns = false;   //下でWidthを指定するので自動調整はOFF
        ...
        {
            if (todoGridView.SelectedRows.Count == 0)   //現在、選択されている行の数をCount == 0なら
            {
